...
namespace ModisaApp.Shared.Repositories
{
    public class HttpServiceProvider(HttpClient _HttpClient, HttpResponseExceptionHander _HttpResponseExceptionHander/*, ITokenProvider _TokenProvider*/) : IHttpServiceProvider
    {
        ...
        public async Task<TResponse?> Post<TResponse>(string url, CancellationToken token)
        {
            //await CheckTokenAsync();
            return await GenerateHttpResponseWraper<TResponse>(await _HttpClient.PostAsync(_BaseUrl + url, null, token));
        }
