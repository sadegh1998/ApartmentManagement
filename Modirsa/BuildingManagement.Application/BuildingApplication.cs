...
namespace BuildingManagement.Application
{
    public class BuildingApplication : IBuildingApplication
    {
        ...
        public async Task<OperationResult> Create(CreateBuilding command)
        {
            ...
            if (await _repository.Exsits(x => x.Name == command.Name))
            {
        ...
        {
            var operation = new OperationResult();
            if (await _repository.Exsits(x => x.Name == command.Name && x.Id != command.Id))
            {
                return operation.Failed(ApplicationMessages.Duplicate);
            }
            var building = _repository.Get(command.Id);
            if (building == null)
            {
        ...
        {
            var building = await _repository.Get(Id);
